...
import { ArrowRight, Leaf, ShieldCheck, MapPin, Truck, Users } from 'lucide-react';
            ...
            <section className="px-6 pt-12 pb-20 lg:pt-16 lg:pb-32 max-w-7xl mx-auto flex flex-col lg:flex-row items-center gap-16 relative z-10">
                ...
                <div className="flex-1 flex flex-col items-start space-y-8">
                    ...
                    <div className="inline-flex items-center gap-2.5 px-4 py-1.5 rounded-full bg-white border border-slate-200 shadow-sm">
                        ...
                        <span className="text-xs font-bold text-slate-700 tracking-wide uppercase">Tukar Sampah Jadi Saldo</span>
                    </div>

                    {/* Headline */}
                    <div className="space-y-4">
                        <h1 className="text-5xl lg:text-7xl font-extrabold text-slate-900 tracking-tight leading-[1.1]">
                            Bersihkan Lingkungan, <br />
                            <span className="text-transparent bg-clip-text bg-gradient-to-r from-brand-500 to-emerald-500">
                                Berbuah Tambahan Cuan
                            </span>
                        </h1>
                        <p className="text-lg text-slate-600 max-w-xl leading-relaxed">
                            Jangan buang sampah daur ulang Anda! Lewat Beres App, tukar barang bekas Anda dari rumah menjadi saldo digital, emas, atau sembako. Kurir kami yang akan menjemputnya!
                        </p>
                    </div>
                    ...
                    <div className="flex flex-col sm:flex-row items-center gap-4 w-full sm:w-auto pt-4">
                        ...
                        <a href="#harga-sampah" className="w-full sm:w-auto flex items-center justify-center gap-2 px-8 py-4 rounded-full bg-white text-slate-700 font-semibold border border-slate-200 hover:bg-slate-50 hover:text-brand-600 transition-all shadow-sm active:scale-[0.98] group">
                            Cek Daftar Harga
                    ...
                    <div className="flex items-center gap-6 pt-6 border-t border-slate-200 w-full sm:max-w-md">
                        <div className="flex items-center gap-2">
                            ...
                            <span className="text-xs font-semibold text-slate-500 uppercase tracking-wider">Layanan Jemput Rumah</span>
                        </div>
                        <div className="flex items-center gap-2">
                            <ShieldCheck className="h-5 w-5 text-emerald-500" />
                            <span className="text-xs font-semibold text-slate-500 uppercase tracking-wider">Harga Timbangan Terbuka</span>
                        </div>
                    </div>
                </div>
                ...
                <div className="flex-1 w-full lg:w-auto relative lg:perspective-1000">
                    ...
                    <div className="relative bg-white/90 p-8 rounded-3xl shadow-2xl border border-white grid grid-cols-2 gap-5 backdrop-blur-xl z-10 w-full transform lg:-rotate-2 hover:rotate-0 transition-transform duration-500">
                        {/* Main Stat Card */}
                        <div className="col-span-2 bg-gradient-to-b from-brand-50 to-white border border-brand-100 p-6 rounded-2xl flex items-center justify-between shadow-sm hover:shadow-md transition-shadow">
                            <div>
                                <p className="text-xs font-bold text-slate-500 tracking-wider uppercase mb-1">Total Tabungan Cuan Terkumpul</p>
                                <div className="flex items-baseline gap-2">
                                    <p className="text-4xl font-extrabold text-slate-900 tracking-tight">328</p>
                                    <span className="text-sm font-semibold text-slate-500">Juta Rupiah</span>
                                </div>
                            </div>
                        ...
                        <div className="bg-white border border-slate-100 p-5 rounded-2xl flex flex-col justify-between shadow-sm hover:border-brand-200 transition-colors">
                            <div className="flex items-center gap-2 mb-3">
                                <div className="p-1.5 bg-blue-50 rounded-lg"><Users className="h-4 w-4 text-blue-500" /></div>
                                <p className="text-xs font-bold text-slate-500 uppercase tracking-wider">Nasabah Bergabung</p>
                            </div>
                            <div className="flex items-end flex-wrap gap-2">
                                <p className="text-3xl font-extrabold text-slate-900">4.8k+</p>
                                <p className="text-[10px] font-bold text-emerald-700 bg-emerald-50 px-2 py-1 rounded-md mb-1 border border-emerald-100">Keluarga</p>
                            </div>
                        </div>
                        ...
                        <div className="bg-white border border-slate-100 p-5 rounded-2xl flex flex-col justify-between shadow-sm hover:border-emerald-200 transition-colors">
                            <div className="flex items-center gap-2 mb-3">
                                <div className="p-1.5 bg-amber-50 rounded-lg"><Truck className="h-4 w-4 text-amber-500" /></div>
                                <p className="text-xs font-bold text-slate-500 uppercase tracking-wider">Volume Terdaur Ulang</p>
                            </div>
                            <div className="flex items-baseline gap-1.5">
                                <p className="text-3xl font-extrabold text-slate-900">240</p>
                                <p className="text-xs font-semibold text-slate-500 mb-1">Ton</p>
                            </div>
                        </div>
                    </div>
                </div>
